...
class Cfg:
    ...
    eps: float = 1e-8
    normalize: bool = True  # L2-normalize rows before similarity
    detach_negatives: bool = True  # do not backprop through negatives
    contrastive_weight: float = 0.1  # Weight for contrastive term


class InfoNCELocal:
    """
    InfoNCE-style goodness adapted for FF framework.
    Goodness = sum(h^2) + contrastive_weight * InfoNCE_score
    Uses sum of squares as base with InfoNCE as regularizer.
    """
    ...
    def goodness(self, h: torch.Tensor, layer_index: int, state: Optional[Dict]) -> torch.Tensor:
        ...
        # InfoNCE contrastive term with numerical stability
        x = h
        ...
        sim = sim - sim.max(dim=1, keepdim=True).values  # stability per row
        ...
        try:
            ...
            if not torch.all(torch.isfinite(logsumexp)):
                return base_goodness
        except:
            return base_goodness
        
        pos = torch.diagonal(logits)
        contrastive_score = pos - logsumexp
        ...
        # Combine: base goodness + scaled contrastive score
...
@register("info_nce_local")
def build(cfg: DictConfig) -> InfoNCELocal:
    return InfoNCELocal(Cfg(**getattr(cfg, "params", {})))
