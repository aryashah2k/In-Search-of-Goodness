...
class FF_model(torch.nn.Module):
    ...
    def __init__(self, opt):
        ...
        # Initialize goodness function
        goodness_name = getattr(opt.model, 'goodness_function', 'sum_of_squares')
        self.goodness_fn = goodness.get_goodness_function(goodness_name, opt)
        self.goodness_states = {}
        for i in range(self.opt.model.num_layers):
            self.goodness_states[i] = self.goodness_fn.init_state(
                layer_index=i,
                width=self.num_channels[i],
    ...
    def _calc_ff_loss(self, z, labels, layer_idx):
        # Use the pluggable goodness function
        goodness_values = self.goodness_fn.goodness(
            z, 
            layer_index=layer_idx, 
            state=self.goodness_states.get(layer_idx)
        )

        # Threshold is the average dimension (e.g., 2000 for hidden layers)
        logits = goodness_values - z.shape[1]
    ...
    def forward(self, inputs, labels):
        ...
        for idx, layer in enumerate(self.model):
            ...
            # Update goodness function state (for stateful functions)
            self.goodness_states[idx] = self.goodness_fn.update_state(
                self.goodness_states[idx], z
            )
            ...
            ff_loss, ff_accuracy = self._calc_ff_loss(z, posneg_labels, idx)
